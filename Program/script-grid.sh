...
do
    sbatch --export=instance=${instance},time=${instances_time[$external_index]},crossover=${crossoverType} exec-grid.sh
...
# Crossover L-OX - type 4. 
# Infos:
# - it's the same as crossovertype4; 
# - correlated vertices are reseted each 500 iterations; 
# - if correlatedVertices[i] < nbGranular, we complete using non-inserted closest vertices)
external_index=0
crossoverType=7
for instance in "${instances[@]}"
do
...
# - it's the same as crossovertype4; 
# - correlated vertices are reseted each 500 iterations; 
# - if correlatedVertices[i] < nbGranular, we don't complete it
external_index=0
crossoverType=8
for instance in "${instances[@]}"
do
