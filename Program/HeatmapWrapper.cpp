...
void HeatmapWrapper::processBigTourDPDP(Population *&pop)
{
    ...
    {
        command = "cd heatmap/ && python generate_data.py --problem vrp --name train --seed 1 -f --dataset_size 10";
    ...
    command = "cd heatmap/ && CUDA_VISIBLE_DEVICES=0,0 python export_heatmap.py --problem vrp --checkpoint logs/vrp_" + instanceName + "/best_val_loss_checkpoint.tar --instances data/vrp/" + instanceName + "_test_seed3.pkl -f --batch_size 1";
    res = system(command.c_str());
    std::cout << "Step #3 - export heatmap." << res << std::endl; 


    command = "cd heatmap/ && CUDA_VISIBLE_DEVICES=0,0 python eval.py data/vrp/" + instanceName + "_test_seed3.pkl --problem cvrp --decode_strategy dpdp --score_function heatmap_potential --beam_size 10000 --heatmap_threshold 1e-5 --heatmap results/vrp/" + instanceName + "_test_seed3/heatmaps/heatmaps_vrp_" + instanceName + ".pkl -f";
    res = system(command.c_str());
